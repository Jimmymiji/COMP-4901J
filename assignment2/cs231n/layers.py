...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # std,mean,x,x_normed,eps,gamma = cache
    # N,D = dout.shape
    # dgamma = np.sum(x_normed*dout,axis=0,keepdims=True)
    # dbeta = np.sum(dout,axis=0,keepdims=True)
    # dx = ((1-1/N)*np.sqrt(std*std+eps) - (x-mean)*(0.5*1/np.sqrt(std*std+eps))*(2*((x-mean)-np.sum(x-mean,axis=0)/N)/N))/(std*std + eps)
    # dx*=dout
    # dx*=gamma
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = dout*gamma*invstd-dgamma*gamma*(invstd**2)*(xmean)/N - dbeta*gamma*invstd/N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
        ...
        for c in range(C):
            for h in range(H_out):
                for w in range(W_out):
                    ...
                    w1 = w*stride
                    w2 = w1 + pool_width
                    block = x[n, c, h1:h2, w1:w2]
                    maxindex = np.unravel_index(block.argmax(), block.shape)
                    blockchoice = np.zeros(block.shape)
                    blockchoice[maxindex] = 1
                    dx[n,c,h1:h2,w1:w2] += blockchoice*dout[n,c,h,w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dx, dgamma, dbeta = batchnorm_backward(dout.transpose(0,3,2,1).reshape(-1, C), cache)
